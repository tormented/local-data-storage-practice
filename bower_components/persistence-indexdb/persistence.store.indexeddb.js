...
persistence.store.websql.config = function(persistence, dbname, description, size) {
	...
	} else if (window && window.indexedDB){
		persistence.db.implementation = "lawnchair";
	} else {
		try {
			if (openDatabaseSync) {
	...
	persistence.db.lawnchair = {};

	persistence.db.lawnchair.connect = function (dbname) {
		var that = {};
		var conn = openIndexedDB();

		that.transaction = function (fn) {
			return conn.transaction(function (sqlt){
				return fn(persistence.db.lawnchair.transaction(sqlt));
			});
		};
		return that;
	};

	persistence.db.lawnchair.transaction = function (t) {
		var that = {};
		that.executeSql = function (query, args, successFn, errorFn) {
			if(persistence.debug) {
				console.log(query, args);
	...
	persistence.db.connect = function (dbname, description, size) {
		...
		} else if (persistence.db.implementation == "lawnchair"){
			return persistence.db.lawnchair.connect(dbname);
		}
	};
